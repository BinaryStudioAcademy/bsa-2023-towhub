...
import { HttpCode, HttpError, HttpErrorMessage } from '~/libs/packages/http/http.js';
...
class BusinessController extends Controller {
    ...
    private async create(
        ...
        const existingBusiness = await this.businessService.findByOwnerId(options.user.id);
    ...
    private async update(
        ...
        if (!businessToUpdate) {
            throw new HttpError({ status: HttpCode.BAD_REQUEST, message: HttpErrorMessage.BUSINESS_DOES_NOT_EXIST });
        }
    ...
    private async delete(
        ...
        const deletionResult = await this.businessService.delete(businessToDelete.id);
